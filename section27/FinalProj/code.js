...
function getStartEndRandNumber() {
  const startNum = document.getElementsByName('rand1')[0];
  const endNum = document.getElementsByName('rand2')[0];
  const result = getRandomNumber(startNum.value, endNum.value);
  document.getElementById('startend-randno').innerHTML = result;
}
function getNumberOfWords(number) {
  const wordArr = lorem.split(' ');
  let result = '';
...
function setLoremString() {
  document.getElementById('set-full-string').innerHTML = lorem;
}
...
function getLoremIpsumOnChars() {
  ...
  const noOfAvgLettersPerWord = 7;
  const noOfWords = Math.floor(noOfChars / noOfAvgLettersPerWord);
  document.getElementById('chars-lorem').innerHTML = getNumberOfWords(noOfWords);
}
function getFormattedDate() {
  const date = new Date().toLocaleString('en-uk');
  document.getElementById('formatted-date').innerHTML = date;
}
function getCurrentTime() {
  const date = new Date();
  document.getElementById('current-time').innerHTML = `${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`;
}
function convertInchesToFeet() {
  // feet = inches / 12;
  const inches = document.getElementById('inches-to-feet').value;
  document.getElementById('display-feet').innerHTML = inches / 12;
}
function convertFeetToInches() {
  const ft = document.getElementById('feet-to-inches-input').value;
  document.getElementById('display-inches').innerHTML = ft * 12;
}

function doWordLengthComparison() {
  const word1 = document.getElementById('word1').value;
  const word2 = document.getElementById('word2').value;
  if (word1.length === word2.length) {
    document.getElementById('word-compare-res').innerHTML = 'Words are equal';
  } else {
    document.getElementById('word-compare-res').innerHTML = 'Words are not equal';
  }
}
...
function getRandPassword() {
  ...
  document.getElementById('rand-pass-res').innerHTML = result;
}

function isNoEvenOrOdd() {
  const number = document.getElementById('even-odd').value;
  const resultElement = document.getElementById('even-odd-res');
...
const colors = ['green', 'yellow', 'blue', 'purple', 'cyan', 'magenta', 'white', 'coral', 'red', 'lightred', 'lightgrey'];

function getRandomColor() {
  document.getElementById('random-color-result').innerHTML = colors[getRandomNumber(0, colors.length)];
}
...
function getRandFirstAndLastName() {
  const randNo1 = getRandomNumber(0, names.length);
  const randNo2 = getRandomNumber(0, names.length);
  const result = `${names[randNo1].firstName} ${
    names[randNo2].lastName}`;
  document.getElementById('rand-first-last-res').innerHTML = result;
}
